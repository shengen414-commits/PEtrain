...
##### windows端-蓝牙设置里开端口，脚本根据gpx发送模拟定位信息
    ...
    -电脑端装好所有环境后运行python main.py 

##### 手机端-BluetoothGNSS接收，虚拟机内运行需要定位的软件，读取BluetoothGNSS软件传来的模拟定位
    -GooglePlay下载BluetoothGNSS、光速虚拟机
    -手机蓝牙连接电脑，已配对状态就可以
    -手机宿主机开开发者模式，模拟定位应用选BluetoothGNSS，此后打开这个软件，根据app引导做，最后右下角Settings-device选择发射信息的电脑，然后回到Connect连接上，电脑运行main.py后，点右下角蓝牙图标连接，显示出传来的数据即可。小窗化挂在前台可以避免后续用虚拟机的时候被后台杀掉。
...
按键精准释放：通过监听键盘指令（默认 w 键），以 1Hz 频率连续释放设定时长（如 10 分钟）的连续数据段，支持启动自动无限循环闭环。
...
folium：用于生成交互式地图，供电脑端预览轨迹和定位点改变。
